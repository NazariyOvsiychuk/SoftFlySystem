...
import { buildAnalyticsDeepDive } from "@/lib/operations-admin";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const payload = await buildAnalyticsDeepDive(body.start, body.end);
    return NextResponse.json(payload);
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Не вдалося завантажити аналітику." },
      { status: 400 }
    );
  }
}
